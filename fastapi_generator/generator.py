"""Générateur principal pour les applications FastAPI"""
...
class FastAPIGenerator:
    ...
    def from_existing_project(cls, project_path: str = ".") -> "FastAPIGenerator":
        ...
        config_path = Path(project_path) / ".fastapi-gen.json"
    ...
    def generate(self) -> None:
        ...
        if self.project_path.exists() and not self.force_overwrite:
            raise ValueError(f"Le répertoire '{self.project_name}' existe déjà. Utilisez --force pour le remplacer.")
    ...
    def add_router(self, router_name: str, model_name: Optional[str] = None, include_crud: bool = False) -> None:
    ...
    def add_model(self, model_name: str, fields: Dict[str, str]) -> None:
        ...
        self.template_manager.render_model(
            self.project_path,
            model_name,
            fields
        )
